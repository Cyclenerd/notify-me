...
use App::Options (
	option => {
		...
		issue    => { required => 1, description => "The issue number", type => "integer" },
...
my $issue_number = $App::options{issue};
...
my $request = POST "https://api.github.com/repos/$owner/$repo/issues/$issue_number/comments";
$request->header(
